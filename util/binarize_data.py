...
def binarize_data(data_path: str) -> list[dict]:
    ...
    for entry in data:
        ...
        if len(options) < 2:
            logging.info(
                f"Skipping entry with insufficient options: {entry['problem']}"
            )
            continue
...
data_paths: list[str] = [
    arithmetic_data_path,
    scientific_data_path,
    causal_data_path,
    commonsense_data_path,
]
...
new_commonsense_data_path = (
    "data/commonsense reasoning/commonsense_qa_processed_binarized.json"
)

new_data_paths = [
    new_arithmetic_data_path,
    new_scientific_data_path,
    new_causal_data_path,
    new_commonsense_data_path,
]
